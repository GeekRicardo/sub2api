  ...
  >
    ...
    >
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.useCodexUserAgent') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.useCodexUserAgentDesc') }}
            </p>
          </div>
          <button
            type="button"
            data-testid="openai-use-codex-user-agent-toggle"
            @click="openaiUseCodexUserAgent = !openaiUseCodexUserAgent"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openaiUseCodexUserAgent ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openaiUseCodexUserAgent ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const openaiUseCodexUserAgent = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openaiUseCodexUserAgent.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    ...
    openaiUseCodexUserAgent.value = extra?.openai_use_codex_user_agent === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'apikey')) {
      ...
      if (openaiUseCodexUserAgent.value) {
        newExtra.openai_use_codex_user_agent = true
      } else if (currentExtra.openai_use_codex_user_agent === true) {
        newExtra.openai_use_codex_user_agent = false
      } else {
        delete newExtra.openai_use_codex_user_agent
      }
